...
#   df_a2 <- df_a |>
#     mutate(.id_a = dplyr::row_number())

#   df_b2 <- df_b |>
#     mutate(.id_b = dplyr::row_number())
...
#   candidates <- df_a2 |>
#     select(.id_a, all_of(block_vars), name.a = !!name_a_sym) |>
#     inner_join(
#       df_b2 |>
#         select(.id_b, all_of(block_vars), name.b = !!name_b_sym, everything()),
#       by = block_vars
#     )
...
#       jw_full = 1 - stringdist(norm_a, norm_b, method = "jw"),
#       lv_full = 1 - stringdist(norm_a, norm_b, method = "lv"),
...
#   # keep only good matches and top_n per .id_a
#   scored |>
#     filter(match_score >= min_score) |>
#     group_by(.id_a) |>
#     slice_max(order_by = match_score, n = top_n, with_ties = FALSE) |>
#     ungroup() |>
#     arrange(.id_a, dplyr::desc(match_score))
# }
