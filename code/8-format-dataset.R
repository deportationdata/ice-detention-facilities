...
facility_formatted <-
  ...
  mutate(
    name = name |>
      ...
      # straight ASCII ' that's common in source data, otherwise downstream
      # apostrophe-aware fixes (cap-after-apostrophe, possessive-s undo) miss
      # half of them and you get "Sheriff'S Office" output.
      str_replace_all("[‘’`']", "’") |>
      str_squish() |>
      ...
      tools::toTitleCase() |>
      # capitalize letter after apostrophe (O'hare -> O'Hare). Line 55 above
      # normalizes all apostrophes to the curly form (’), so this pattern
      # must match curly, not straight.
      ...
      # fix possessive 's — the single-letter step above capitalizes it because
      # the curly apostrophe is treated as a word boundary. Pattern uses curly
      # since line 55 normalized everything to that form.
      ...
      # expand SO -> Sheriff's Office when preceded by Parish/County. Use the
      # curly apostrophe to match the rest of the pipeline post-normalization.
      str_replace_all(
...
# XX-prefix codes are internal placeholders for facilities that have no real
# ICE code yet — XXWICHI (hand-curated for Wichita County Jail) and synthetic
# codes generated by 0-match-website-new-facilities.R for unmatched website
# facilities. Blank them in the final outputs so downstream users see no code
# rather than the internal marker; the rows themselves stay so the facility
# info is still visible.
facility_formatted <-
  facility_formatted |>
  mutate(
    detention_facility_code = if_else(
      str_starts(detention_facility_code, "XX"),
      NA_character_,
      detention_facility_code
    )
  )
