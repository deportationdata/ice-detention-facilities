...
dropdown <- remDr$findElement(
  using = "id",
  value = "edit-field-foia-category-target-id"
)
...
contract_option <- remDr$findElement(
  using = "xpath",
  value = "//select[@id='edit-field-foia-category-target-id']/option[contains(text(),'Detention Facility Contracts')]"
)
contract_option$clickElement()
Sys.sleep(1)

# click Apply button
apply_button <- remDr$findElement(
  using = "id",
  value = "edit-submit-foia-request-library-content-"
)
  ...
  next_button <- tryCatch(
    {
      remDr$findElement(
        using = "xpath",
        value = "//a[contains(@aria-label, 'Next page')]"
      )
    },
...
inspection_option <- remDr$findElement(
  using = "xpath",
  value = "//select[@id='edit-field-foia-category-target-id']/option[contains(text(),'Office of Detention Oversight')]"
)
...
arrow::write_parquet(
  contracts_and_inspections,
  "data/contracts-and-inspections.parquet"
)
